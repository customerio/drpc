...
// protoc-gen-go-drpc generates DRPC code for protobuf services with sync pool optimization.
package main
...
type config struct {
	protolib string
	json     bool
	syncpool bool // NEW: Flag to enable sync pool optimization
}
...
func generateFile(plugin *protogen.Plugin, file *protogen.File, conf config) {
	...
	d := &drpc{gf, file, conf} // Pass config to drpc struct
...
type drpc struct {
	*protogen.GeneratedFile
	file *protogen.File
	conf config // NEW: Store config for sync pool feature
}
...
// NEW: Helper function to generate sync pool variable name
func (d *drpc) PoolName(typeName string) string {
...
func (d *drpc) generateService(service *protogen.Service) {
	// NEW: Generate sync pools for each unique response type if enabled
	if d.conf.syncpool {
		d.generateSyncPools(service)
	}
...
// NEW: Generate sync pools for response types
func (d *drpc) generateSyncPools(service *protogen.Service) {
	...
	for _, method := range service.Methods {
		if !method.Desc.IsStreamingServer() && !method.Desc.IsStreamingClient() {
			...
			responseTypes[outType] = true
		}
		// Also collect input types for streaming
		if method.Desc.IsStreamingClient() {
			inType := d.InputType(method)
			responseTypes[inType] = true
		}
	}

	// Generate both private and public sync pools for each unique type
...
func (d *drpc) generateClientMethod(method *protogen.Method) {
	...
	if !method.Desc.IsStreamingServer() && !method.Desc.IsStreamingClient() {
		if d.conf.syncpool {
			// NEW: Use sync pool - return pooled object directly, no copying!
			poolName := d.PoolName(outType)
			d.P("out := ", poolName, ".Get().(*", outType, ")")
			d.P("// Reset the object to clean state")
			d.P("if resetter, ok := interface{}(out).(interface{ Reset() }); ok {")
			d.P("resetter.Reset()")
			d.P("}")
			d.P("err := c.cc.Invoke(ctx, ", d.RPCGoString(method), ", ", d.EncodingName(), "{}, in, out)")
			d.P("if err != nil {")
			d.P("// Return to pool on error")
			d.P(poolName, ".Put(out)")
			d.P("return nil, err")
			d.P("}")
			d.P("// WARNING: Caller must return object to pool when done!")
			d.P("// Use ", strings.TrimPrefix(outType, "*"), "Pool.Put(result) when finished")
			d.P("return out, nil")
		} else {
	...
	if genRecv {
		...
		if d.conf.syncpool {
			// NEW: Use sync pool for streaming receive - no copying!
			poolName := d.PoolName(outType)
			d.P("m := ", poolName, ".Get().(*", outType, ")")
			d.P("// Reset the object to clean state")
			d.P("if resetter, ok := interface{}(m).(interface{ Reset() }); ok {")
			d.P("resetter.Reset()")
			d.P("}")
			d.P("if err := x.MsgRecv(m, ", d.EncodingName(), "{}); err != nil {")
			d.P("// Return to pool on error")
			d.P(poolName, ".Put(m)")
			d.P("return nil, err")
			d.P("}")
	...
	if genCloseAndRecv {
		...
		d.P("if err := x.CloseSend(); err != nil { return nil, err }")
		if d.conf.syncpool {
			// NEW: Use sync pool for close and receive
			poolName := d.PoolName(outType)
			d.P("m := ", poolName, ".Get().(*", outType, ")")
			d.P("defer func() {")
			d.P("if resetter, ok := interface{}(m).(interface{ Reset() }); ok {")
...
func (d *drpc) generateServerMethod(method *protogen.Method) {
	...
	if genRecv {
		...
		if d.conf.syncpool {
			// NEW: Use sync pool for server streaming receive
			poolName := d.PoolName(d.InputType(method))
			d.P("m := ", poolName, ".Get().(*", d.InputType(method), ")")
			d.P("defer func() {")
			d.P("if resetter, ok := interface{}(m).(interface{ Reset() }); ok {")
			d.P("resetter.Reset()")
			d.P("}")
